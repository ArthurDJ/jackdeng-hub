...
export async function buildSidebarData(options: SidebarOptions = {}) {
  ...
  const [categoriesResult, tagsResult, recentResult, allPublished] = await Promise.all([
  ...
  // Build archive entries grouped by year+month
  const monthCounts: Record<string, number> = {}
  for (const blog of allPublished.docs as any[]) {
