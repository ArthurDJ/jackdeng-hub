...
| 生产库结构与迁移文件不一致 | 🟡 中 | ✅ 已修复 (#53) | #46 修掉的开发模式推送留下的历史偏差。**先确定以谁为准**：三个库两两比对——只跑迁移的一次性库、把当前代码 schema 推进空库的一次性库、生产库。结果：**生产与代码完全一致**（列、约束、索引名、枚举），落后的只是迁移链，差 26 处：4 个 select 字段是 varchar 而非枚举、`tool_runs.detail` 是 text、`run_at` 多了 `now()` 默认值、`blogs.status` 可空、5 个索引名/形状不同。注意 Payload 的 schema 快照只到 04-10，之后的迁移都是手写的，`migrate:create` 会以错误基线做差异，所以没用它。新迁移 `20260924_000001_align_schema_with_code` 每一步都先检查再动手，生产上是空操作。**彩排**：迁移链库写入真实形态数据后 up → 与代码 schema **完全一致**、数据原样（类型变为枚举）；down → 与迁移前快照**完全一致**；再 up → 一致；在与生产同构的库上连跑两次 up() → 零变化。 |
...
| 博客列表类页面每次请求都现场渲染 | 🟡 中 | ✅ 已修复 (#51) | #48 之后 `[locale]` 下只剩博客列表、分类、标签、归档仍是 `ƒ`：它们读 `searchParams`（`?page=N` / `?year=&month=`），读了就强制动态渲染。分页改为路径段 `/blog/page/N`（分类、标签同理），第 1 页留在无页码 URL，`/page/1` 308 回去，页码严格校验（`src/lib/pagination.ts`），超出范围真 404——新路由放在 `loading.tsx` 之外（#34）。三个列表页各抽一个 `view.tsx` 供两条路由共用。旧 `?page=N`（N≥2）在 `next.config` 永久重定向。归档页不再筛选，侧边栏月份链接改为页内锚点，月份名改用 `Intl`（原为写死的英文数组，中文页也显示 September）。验证：临时把每页篇数改成 1 构建，真实第 2–4 页 `MISS`→`HIT`、越界 404、重定向 308；改回 12 后全部 `HIT`。 |
| 博客链接不带语言前缀；首页项目卡片 404 | 🔴 高 | ✅ 已修复 (#52) | 两个问题，同一个根源——`next/link` 与 next-intl `Link` 混用。① BlogCard、TagBadge、CategoryBadge、Sidebar、文章页面包屑用 `next/link` 写 `/blog/xxx`，每次点击经 middleware 多一次 307，禁用 cookie 时按 Accept-Language 落到另一种语言；改用 next-intl 的 `Link`，文章页的「切换语言」链接改用 `locale` 属性。② **首页「精选项目」4 张卡片在生产 404 了约 5 个月**（2026-04-16 fb40d3f 起）：HomeProjectCard 用的是 next-intl 的 `Link`，href 却又手写了 `/${locale}/`，生成 `/en/en/projects/…`。对一个要代替简历的首页，这是招聘方最可能点的链接。**冒烟检查一直没发现**，因为它只 `curl -L` 一份固定 URL 清单，从不点页面上的链接。新增冒烟步骤：抓 sitemap 全部页面、不跟随重定向检查每个站内链接（双前缀 / 无前缀 / 非 200 即失败）。验证：对当时的生产跑报 36 处失败、exit 1；对修复后的构建 42 页 46 个链接全部通过、exit 0。 |
| 迁移与代码 schema 的偏差无人检查 | 🟡 中 | ✅ 已修复 (#54) | #53 修掉的 26 处偏差持续了 5 个月，因为从没有东西比较「迁移建出来的库」和「代码定义的 schema」。`typecheck` job 加一个 Postgres service 与三个步骤：迁移建库 → 把代码 schema 推进另一个空库 → 逐列/约束/索引/枚举比对，有差异即失败（`scripts/schema-drift.ts`）。仍是 `typecheck` job 的步骤而非新 job，理由同 i18n 检查。为此 `push` 从恒为 false 改为「`PAYLOAD_SCHEMA_PUSH=1` **且** 地址是 localhost」——守卫抽成 `isLocalDatabaseUrl()` 并单测（含 `localhost.evil.com`、userinfo 里塞 `@localhost` 等伪装），任何环境变量组合都不能再推到远程库。**验证它会红**：当前代码通过（177 列 / 48 约束 / 93 索引 / 10 枚举全一致）；移走 #53 的对齐迁移 → 报 **26** 处；给 Projects 加一个字段不写迁移 → 报出该列。同时删除了生产 `payload_migrations` 里 04-04 起的 `batch = -1` dev 记录（事务内、行数必须为 1），此后 `payload migrate` 不再弹「dev mode」确认。 |
| 搜索引擎和分享预览不知道「这是谁」 | 🟡 中 | ✅ 已修复 (#55) | 首页与 /about 没有任何 JSON-LD，搜索引擎无法把本站、GitHub、LinkedIn 关联到同一个人；分享卡片只有一行「Jack Deng」，发到 LinkedIn / 微信看不出做什么。首页加 schema.org `Person`，/about 加 `ProfilePage`（`mainEntity` 为同一 Person），数据取自 `profile.ts`——只用站上已公开的信息，**刻意不写 `jobTitle`**（定位语是方向不是职位，放 `description`）**也不写 email**。新增 `toJsonLd()` 把 `<` 转义，值里带 `</script>` 也关不掉标签。`/og` 加可选 `subtitle`，首页与 /about 传当前语言的定位语，全站默认卡片也带上；title / subtitle 截到 120 字，这个接口会把查询参数原样画进本域名的图片。验证：四个页面 JSON-LD 可解析、字段齐全且无 jobTitle / email；中英卡片实际渲染正确；不带 subtitle 的旧用法不变；链接检查 42 页 46 链接全过。 |
| 浅色模式看不清 | 🔴 高 | ✅ 已修复 (#56) | 浅色是跟随系统时的默认主题，却从一开始就是坏的。**顶栏**：深色背景写死在内联样式里，浅色覆盖规则选的是 `header[data-navbar]`，属性却加在 `<nav>` 上，所以从没生效——白底页面顶着一条深灰栏，上面的名字是近黑色，对比度 1.6:1；手机端展开菜单同样是深底深字。**次要文字**：`--text-tertiary` #999（日期、地点、页脚、技能分组）2.6–2.9:1，阅读时长用的是 `--text-disabled` #ccc，1.5:1。**徽章**：项目/工具状态的颜色是只为深色调过的十六进制值，复制在六个文件里（「Active」2.2:1）；博客标签直接拿品牌色当字色，JavaScript 黄 1.3:1、React 青 1.5:1。深色模式问题小一些：tertiary 3.7–4.1:1，白字按钮（简历下载、跳转链接）在 #3b82f6 上 3.7:1。改法：顶栏与菜单改用主题变量；调深/调亮 tertiary、accent、status 色值；白字统一放在新的 `--accent-solid` / `--danger-solid` 上；状态色收进 `src/lib/statusColors.ts`（`color-mix` 生成底色，替换掉只能配十六进制的 `color + '18'` 拼接）；标签字色 = 品牌色 35% 混入正文色，保留色相。顺手修掉「View project → →」双箭头。验证：两套主题下 13 个页面逐个文字节点算对比度，全部 ≥ 4.5:1（大字 ≥ 3:1），修改前线上浅色首页有 11 组不达标；84 个中英页面与线上的可见文字 diff 只有去掉的箭头。 |
| 未知路径的 404 | 🟡 中 | ✅ 已修复 (#57) | 两个问题。**① 带点的路径答 500**：`/wp-login.php`、`/.env`、`/foo.txt` 这类扫描器天天打的路径不经过 middleware（matcher 排除了带点的路径），于是被 `[locale]` 当成 locale = "foo.txt"；layout 在 `setRequestLocale` 之前就 `notFound()`，404 页的翻译只好退回去读请求头，静态路由不允许，整页 500。修复前生产上四个都是 500。改为先把 locale 设成默认值再 `notFound()`。**② locale 下的未知路径没有站点外壳**：`/en/nope` 没有路由认领，落到根 `not-found.tsx`——深色、无导航、不跟主题、不翻译。新增 `[locale]/[...rest]` 调 `notFound()`，交给带导航和主题的 `[locale]/not-found.tsx`，标题本地化并加 `noindex`。顺带去掉 404 页自带的页脚——layout 已经渲染了一个，之前是两个。冒烟检查加两条反向探针（locale 下多级未知路径、带 `.php` 的路径）。验证：21 条路径状态码逐一核对，84 个中英页面与线上文字 diff 为 0。 |
| CSP 只有 frame-ancestors | 🟡 中 | 🔄 试运行 (#58) | #49 的 CSP 只防被嵌套，脚本、图片、表单提交去向一概不管。完整策略以 `Content-Security-Policy-Report-Only` 上线：浏览器只报告不拦截，违规 POST 到新的 `/api/csp-report`，每条记成一行 `[csp]` 日志（Vercel 函数日志）。允许的外部来源只有两个：Turnstile（`challenges.cloudflare.com`，脚本 / iframe / 接口）与 Vercel Blob 图床。`script-src` 保留 `'unsafe-inline'`——Next 以内联 `<script>` 分块流式输出页面，改用 nonce 会让所有缓存页面每次请求重渲染；即便如此仍然挡住：其他域名的脚本、插件、`<base>` 劫持、表单外发、第三方 iframe。上报接口不鉴权（浏览器自动发），所以什么都不信：超过 16 KB 直接 413、每次最多记 20 条、URL 只留 origin + path。顺带把博客与项目详情页的 JSON-LD 也换成 #55 的 `toJsonLd()`（转义 `<`），`'unsafe-inline'` 在，`</script>` 注入就还有意义。验证：解析器 11 个测试、8 个变异全部被抓；本地 `next start` 逐页浏览（首页、About、博客、文章、项目、工具、404、admin 登录页）零违规；故意加载外域图片，报告送达并按预期去掉查询串；5 个页面 JSON-LD 与线上逐字节一致。**待办**：观察一段时间日志（尤其 /admin 登录后、评论区 Turnstile），无真实违规后改为强制执行。 |
| 归档/分类/标签页缺 canonical 与 hreflang | 🟡 中 | ✅ 已修复 (#59) | 全站元数据审计（sitemap 全部 84 个中英页面）：博客首页、文章、项目、工具都有 canonical 与 hreflang，但归档、6 个分类、21 个标签页（中英共 44 页）一个都没有，搜索引擎无从知道中英两版是同一页的两种语言。新增 `localeAlternates()`，分页页面沿用 /blog 的规则——每页自己是 canonical。另外三处：归档页的 description 是写死的英文，中文页也是；标签没有做多语言，中文标签页的 description 直接是英文（如「Oracle NetSuite ERP」），改为中文页用翻译模板、英文页模板 + 标签描述；工具详情页自己设了 `openGraph` 却没带图片，而页面级 `openGraph` 会整个覆盖 layout 的，分享出去没有卡片——补上 `/og` 卡片。验证：本地 `next start` 重跑审计，44 页全部带正确 canonical 与 hreflang，重复 description 从 21 组降到 0；工具卡片图 200；84 页可见正文与线上 diff 为 0（差异只在 `<head>`）。剩下的只是内容：两篇文章标题超过 65 字符、一个项目描述 195 字符，属于文案，未改。 |
| 每页都带着用不到的代码和字体 | 🟢 低 | ✅ 已修复 (#60) | 测量线上首页：JS 207 KB（gzip，15 个文件），另预加载两个各约 70 KB 的字体。可以挪走的三样：① **sonner**（toast 库，约 13 KB gzip）挂在 locale layout 上，每一页都下载，而全站唯一会弹 toast 的是文章页评论提交失败——`<Toaster />` 改挂到文章页评论表单旁；② **搜索面板**只在按 ⌘K / 点搜索按钮时才用，却随每页首屏一起下载——拆出 `CommandPaletteHost`（只含快捷键与状态订阅），面板本体首次打开时 `next/dynamic` 按需加载（约 4 KB gzip），快捷键从面板里移出，避免一次按键切换两次；③ **Geist Mono** 由 `geist` 包声明为 preload，每页一开始就和主字体、脚本抢带宽下载 71 KB，只为日期和小标签——在 `src/lib/fonts.ts` 自行声明同一字体，`preload: false`，其余配置与包内一致。结果（本地 `next start`，gzip）：首页 JS 219.2 → 209.7 KB，About / 项目 / 工具页各少约 9.4–10.3 KB，文章页持平（它需要 sonner）；所有页面首屏只预加载一个字体。验证：真实按键测 ⌘K 打开 / 再按关闭 / Esc 关闭 / 搜索按钮打开、搜「sand」出结果；文章页有 toast 容器、其他页没有；拦截 fetch 模拟评论提交失败，toast 正常弹出（请求未发出）；84 页正文与线上 diff 为 0，差异只有少掉的那条字体 preload。 |
| 按 ⌘K 后立刻打字，字丢了 | 🟡 中 | ✅ 已修复 (#61) | #60 合并后在线上验证时发现：按 ⌘K 紧接着输入「sand」，搜索框是空的。根因早于 #60——面板打开后 50 ms 才 `focus()`，而「打开时清空」的 effect 在首次渲染后才执行，这段时间里敲的字要么没进输入框、要么被清掉；#60 改成首次打开才加载面板，又把这段空窗拉长到一次 chunk 加载。本地测试没发现，是因为测试脚本在按键和输入之间隔了 800 ms。修复：输入框用 `autoFocus`，挂载即获得焦点；清空状态改在关闭时做；面板在浏览器空闲时（`requestIdleCallback`，3 s 兜底）预先挂载——仍不在首屏脚本里（首页 JS 209.8 KB，与 #60 持平），但 ⌘K 时已经就绪。验证：本地 `next start` 上真实按键「⌘K 后立即输入」四个字全部保留并出结果；关闭再开为空且聚焦；↓ + Enter 跳转正常。 |
