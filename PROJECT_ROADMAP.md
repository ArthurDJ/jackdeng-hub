...
| 工具详情页不检查 `accessControl` | 🟡 中 | ⚠️ 未处理 | 查询只过滤 `status != offline`，注释称「权限在渲染层处理」，但渲染层并无检查；local API 默认跳过访问控制，所以 online 的**私有**工具凭 slug 人人可见。当前线上只有一个公开工具，暂不暴露；新增私有工具前需修。#48 已让 `generateStaticParams` 只预渲染公开工具，避免私有工具被烘焙进静态页。 |
