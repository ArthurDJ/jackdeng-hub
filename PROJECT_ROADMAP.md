...
| 博客链接不带语言前缀 | 🟢 低 | ⚠️ 未处理 | BlogCard、TagBadge、CategoryBadge、Sidebar、文章页面包屑用 `next/link` 写 `/blog/xxx`，每次点击经 middleware 多一次 307；禁用 cookie 时按 Accept-Language 落到另一种语言。与 #39 搜索结果链接同类。改用 `@/i18n/navigation` 的 `Link` 即可，单独做。 |
